...
# parameters used for malibu
#   num_frequencies 7
# --lambda_eikonal 10
# --warm_up 500 
# --resample_every 2000
# --lambda_surface_sdf 3000 
# --lambda_surface_normal 5
# --lambda_iso_sdf 1000
# --lambda_iso_normal 600
# --lambda_inter_sdf 100
# --denoise_normal
# --use_off_normal_loss 
...
def run(pointcloud_path, out_dir, decoder_type='siren',
    ...
    batch_size =  8000 #points.shape[0]//2
    ...
    scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, [60000, 130000], gamma=0.5)


    shape = Shape(gt_surface_pts_all.cuda(), n_points=gt_surface_pts_all.shape[1]//3, normals=gt_surface_normals_all.cuda())
    ...
    while True:
        if (it > total_iter):
            ...
            mesh = get_surface_high_res_mesh(
                lambda x: decoder(x).sdf.squeeze(), resolution=400)
        ...
        for batch in data_loader:
            ...
            if (it - kwargs['warm_up']) % 1000 == 0:
                ...
                with torch.autograd.no_grad():
                    ...
                    mesh = get_surface_high_res_mesh(
                        lambda x: decoder(x).sdf.squeeze(), resolution=60)
                    mesh.apply_transform(scale_mat_inv)
            ...
            space_pts = torch.cat(
                [torch.rand_like(tmp.repeat(1,3,1)) * 2 - 1,
                 torch.randn_like(tmp.repeat(1,1,1), device=tmp.device, dtype=tmp.dtype) * 0.2+tmp.repeat(1,1,1)], dim=1)
