...
def run(pointcloud_path, out_dir, decoder_type='siren',
    ...
    shape = Shape(gt_surface_pts_all.cuda(), n_points=gt_surface_pts_all.shape[1], normals=gt_surface_normals_all.cuda())
    ...
    while True:
        if (it > total_iter):
            ...
            mesh = get_surface_high_res_mesh(
                lambda x: decoder(x).sdf.squeeze(), resolution=500)
