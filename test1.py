...
# parameters used  trial 1
#   num_frequencies 9
# --lambda_eikonal 5
# --warm_up 500 
# --resample_every 2000
# --lambda_surface_sdf 3900 
# --lambda_surface_normal 1000
# --lambda_iso_sdf 1000
# --lambda_iso_normal 100
# --lambda_inter_sdf 100
# --denoise_normal

# parameters used  trial 2
#   num_frequencies 10
# --lambda_eikonal 5
# --warm_up 500 
# --resample_every 2000
...
def run(pointcloud_path, out_dir, decoder_type='siren',
    ...
    batch_size = 15000# points.shape[0]//2
    ...
    scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, [50000, 100000], gamma=0.5)


    shape = Shape(gt_surface_pts_all.cuda(), n_points=gt_surface_pts_all.shape[1]//2, normals=gt_surface_normals_all.cuda())
    ...
    while True:
        if (it > total_iter):
            ...
            mesh = get_surface_high_res_mesh(
                lambda x: decoder(x).sdf.squeeze(), resolution=600)
        ...
        for batch in data_loader:
            ...
            if (it - kwargs['warm_up']) % 1000 == 0:
                ...
                with torch.autograd.no_grad():
                    ...
                    mesh = get_surface_high_res_mesh(
                        lambda x: decoder(x).sdf.squeeze(), resolution=300)
                    mesh.apply_transform(scale_mat_inv)
            ...
            space_pts = torch.cat(
                [torch.rand_like(tmp.repeat(1,2,1)) * 2 - 1,
                 torch.randn_like(tmp.repeat(1,1,1), device=tmp.device, dtype=tmp.dtype) * 0.3+tmp.repeat(1,1,1)], dim=1)
...
if __name__ == '__main__':
    ...
    mat_file = '/research/nfs_ertin_1/nithin_data/3D_SAR/neuralImplicitSurfaceSAR/cvdomes_l1_pointCloud/Camry.ply'
    decoder = 'sdf'
    use_sal_loss = True
    out_dir = '/research/nfs_ertin_1/nithin_data/3D_SAR/neuralImplicitSurfaceSAR/experiments/camry_angle'
    resume = False
    warm_up = 500
    ear = True
    num_frequencies = 6
