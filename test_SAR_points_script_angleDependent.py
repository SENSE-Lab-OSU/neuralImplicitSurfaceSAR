...
def load_data_cvdomes(filepath):
    ...
    # read the complex-valued amplitudes used in the image
        ...
        amps = np.concatenate((data_of_interest['real'],data_of_interest['imag']),1)
    # read the complex-valued amplitudes used in the image
...
def run(pointcloud_path, out_dir, decoder_type='siren',
    ...
    modelCombine = CombinedModel(decoder,scatteringNet)
    modelCombine = modelCombine.to(device)
    # training
    total_iter = totalIter
    optimizer = torch.optim.Adam([
                {'params': modelCombine.decoder.parameters()},
                {'params': modelCombine.scatteringNet.parameters(), 'lr': 1e-4}
            ], lr=1e-4, momentum=0.9)
    #optimizer = torch.optim.Adam(decoder.parameters(), lr=1e-4)
    scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, [total_iter//3, total_iter//2], gamma=0.5)
    ...
    while True:
        if (it > total_iter):
            ...
            mesh = get_surface_high_res_mesh(
                lambda x: modelCombine.decoder(x).sdf.squeeze(), resolution=512)
        ...
        for batch in data_loader:
            ...
            modelCombine.train()
            modelCombine.decoder.train()
            scatteringNet.scatteringNet.train()
            ...
            if (it - kwargs['warm_up']) % 1000 == 0:
                ...
                with torch.autograd.no_grad():
                    ...
                    mesh = get_surface_high_res_mesh(
                        lambda x: modelCombine.decoder(x).sdf.squeeze(), resolution=180)
                    mesh.apply_transform(scale_mat_inv)
            ...
            idx = torch.randperm(gt_surface_pts.shape[1]).to(device=gt_surface_pts.device)[:(gt_surface_pts.shape[1])]
...
if __name__ == '__main__':
    ...
    parser.add_argument('mat_file', type=str, nargs=1,
    ...
    if out_dir is None:
        out_dir = os.path.join('exp', 'points_3d_outputs',
                               os.path.basename(mat_file).split('.')[0])
        check_opts = ["eik","isoSDF","onSDF","offSDF","offSal","i","w"]
    ...
    run(mat_file, out_dir, decoder_type=decoder,
        use_sal_loss=use_sal_loss,resume=resume, 
        warm_up=warm_up, weight_mode=args.w, resample_every=args.i,
        ear=ear, lambda_eikonal=args.eik, lambda_iso_sdf=args.isoSDF, 
        lambda_surface_sdf=args.onSDF,lambda_inter_sdf=args.offSDF, 
        lambda_inter_sal=args.offSal,outlier_tolerance=args.outlier_tolerance, 
        num_frequencies=num_frequencies,totalIter = totalIter)
# %%
