...
def run(pointcloud_path, out_dir, decoder_type='siren',
    ...
    # scaling the point-grid representation to fall between -1 and +1
    ...
    if decoder_type == 'siren':
        ...
        # decoder.load_state_dict(loaded_state_dict)
         ...
         # This is the netowrk Strucuture that we use for SAR point-cloud monitoring   
        decoder_params = {
            'dim': 3,
            "out_dims": {'sdf': 1},
            "c_dim": 0,
    ...
    #scatteringNet = scatteringNet.to(device)
    ...
    training_loss_iso_points_sdf =0.0
    training_loss_inter=0.0
    training_loss_normal=0.0
    training_loss_eikonal=0.0
    training_loss_sald =0.0
    training_loss_sdf = 0.0
    training_loss_iso_normal = 0.0
    training_loss_amps = 0.0
    training_loss_ampsISO = 0.0
    training_loss_ampsOFF = 0.0
    ...
    # start training
    save_ply(os.path.join(out_dir, 'in_iso_points.ply'), shape.points.cpu().view(-1,3)*(scale/1.5)+pcenter)
    ...
    while True:
        ...
        for batch in data_loader:
            ...
            gt_combinedInput = gt_combinedInput.to(
                device=device).detach()
            gt_combinedOutput = gt_combinedOutput.to(
                device=device).detach()

            gt_surface_pts,gt_viewVector = torch.split(
            ...
            lambda_surfaceOFF_amps = kwargs['lambda_amps_off']
            ...
            weights = 1.0
            piTorch = torch.tensor(np.pi,device=device)
            if kwargs['warm_up'] >= 0 and it >= kwargs['warm_up']:
                if it == kwargs['warm_up'] or kwargs['resample_every'] > 0 and (it - 
                    ...
                    if kwargs['denoise_normal']:
                        iso_points_normal = denoise_normals(iso_points, iso_points_normal, neighborhood_size=15)
                ...
                training_loss_iso_points_sdf= loss_iso_points_sdf.detach().cpu().numpy()
                ...
                loss_normals = torch.mean((1 - F.cosine_similarity(iso_normals_sampled, iso_g_sampled, 
                    dim=-1).abs())) * kwargs['lambda_iso_normal'] * iso_points_sdf.nelement() / (iso_points_sdf.nelement()+8000)
                ...
                training_loss_iso_normal =loss_normals.detach().cpu().numpy()
                
                


            idx = torch.randperm(gt_surface_pts.shape[1]).to(device=gt_surface_pts.device)[:(
                gt_surface_pts.shape[1])]
            ...
            training_loss_eikonal= loss_eikonal.detach().cpu().numpy()
            # 2. SDF loss
            # loss on iso points
            ...
            if kwargs['warm_up'] >= 0 and it >= kwargs['warm_up'] and kwargs['lambda_iso_sdf'] != 0:
                ...
                loss_sdf = loss_sdf * pred_surface_sdf.nelement() / (pred_surface_sdf.nelement() 
                    + iso_points_sdf.nelement())
            ...
            if kwargs['use_sal_loss'] and iso_points is not None:
                ...
                loss_inter = ((eps_sqrt(dists).sqrt() - pred_space_sdf.abs())**2
                    ).mean() *kwargs['lambda_inter_sal']
            else:
            ...
            loss_normals = torch.mean(
                weights * (1 - F.cosine_similarity(gt_surface_normals, 
                        pred_surface_grad, dim=-1))) * lambda_surface_normal
            if kwargs['warm_up'] >= 0 and it >= kwargs['warm_up'] and kwargs['lambda_iso_normal'] != 0:
                ...
                loss_normals = loss_normals * gt_surface_normals.nelement() / (
                    gt_surface_normals.nelement() + iso_normals_sampled.nelement())

            loss['loss_sdf'] = loss_sdf.detach()
            
            loss['loss_inter'] = loss_inter.detach()
            training_loss_inter= loss_inter.detach().cpu().numpy()
            loss['loss_normals'] = loss_normals.detach()
            training_loss_normal = loss_normals.detach().cpu().numpy()
            loss['loss_sald'] = loss_sald
            loss['loss'] += loss_sdf
            training_loss_sdf = loss_sdf.detach().cpu().numpy()
            loss['loss'] += loss_inter
            loss['loss'] += loss_sald
            training_loss_sald= loss_sald.detach().cpu().numpy()
            loss['loss'] += loss_normals
            ...
            writer.add_scalar("Loss/iso_sdf", training_loss_iso_points_sdf, it)
            writer.add_scalar("Loss/spatial_pts", training_loss_inter, it)
            writer.add_scalar("Loss/normal", training_loss_normal, it)
            writer.add_scalar("Loss/eikonal", training_loss_eikonal, it)
            writer.add_scalar("Loss/salience", training_loss_sald, it)
            writer.add_scalar("Loss/sdf", training_loss_sdf, it)
            writer.add_scalar("Loss/iso_normal", training_loss_iso_normal, it)
            writer.add_scalar("Loss/amps", training_loss_amps, it)
            writer.add_scalar("Loss/ampsISO", training_loss_ampsISO, it)
            writer.add_scalar("Loss/ampsOFF", training_loss_ampsOFF, it)
            it += 1
    writer.close()        
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--lambda_amps', type=float, action=FooAction, dest='amps', default=3e2)
    # parser.add_argument('--lambda_amps_iso', type=float, action=FooAction, dest='amps_iso', default=3e2)
    # parser.add_argument('--lambda_amps_off', type=float, action=FooAction, dest='amps_off', default=1e3)
    ...
    mat_file = '/research/nfs_ertin_1/nithin_data/3D_SAR/cvdomes_8pass_results/Camry.mat'
    decoder = 'sdf'
    use_sal_loss = False
