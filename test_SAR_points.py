...
# --lambda_surface_normal 120
# --lambda_iso_sdf 200
# --lambda_iso_normal 20
...
def run(pointcloud_path, out_dir, decoder_type='siren',
    ...
    # training
    total_iter = 40000
    optimizer = torch.optim.Adam(decoder.parameters(), lr=1.5e-4)
    scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, [10000, 30000], gamma=0.5)


    shape = Shape(gt_surface_pts_all.cuda().repeat(1,20,1), n_points=gt_surface_pts_all.shape[1]*20, normals=gt_surface_normals_all.cuda().repeat(1,20,1))
    ...
    while True:
        if (it > total_iter):
            ...
            mesh = get_surface_high_res_mesh(
                lambda x: decoder(x).sdf.squeeze(), resolution=100)
        ...
        for batch in data_loader:
            ...
            space_pts = torch.cat(
                [torch.rand_like(tmp.repeat(1,30,1)) * 2 - 1,
                 torch.randn_like(tmp.repeat(1,30,1), device=tmp.device, dtype=tmp.dtype) * 0.2+tmp.repeat(1,30,1)], dim=1)
